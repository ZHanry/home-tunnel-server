...
Home Tunnel separates control traffic from proxied application traffic. Caddy is the only public HTTP/TLS entry point. FRPS exposes a separate TLS listener for managed Windows, Linux, macOS, and Android Agents and, only when explicitly enabled, administrator-assigned raw TCP/UDP ports.
...
flowchart LR
    ...
    Client["Windows, Linux, macOS or Android client + managed Agent"] -->|"REST + WebSocket"| Caddy
...
On Android, the native Agent is packaged inside the signed APK/AAB and loaded by
the application; the client does not download and execute arbitrary native
binaries. Long-running operation uses a visible foreground service. Android
battery optimization and OEM background restrictions remain outside the server
trust boundary and can interrupt an Experimental client even when its lease and

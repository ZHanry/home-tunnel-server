...
生产目前固定使用 FRP 0.62.1。0.70.1 的隔离兼容矩阵已经覆盖 TLS、授权插件、HTTP/TCP 隧道、Windows/Linux 渲染和白名单，但它不是只改版本号就能升级：Agent API 有变更，而且受保护环境中的 Windows Agent 二进制与可信 SHA-256 仍对应 0.62.1。因此生产不会提前切换。
...
Home Tunnel 仍处在需要真实部署验证的阶段。它没有公开动态演示；macOS 是 Beta；Windows 是仅源码 Experimental；生产 FRP 仍为 0.62.1。它更适合愿意维护 Linux 服务器、域名、DNS 与备份的人，不适合希望零运维 SaaS、匿名公共代理或任意协议转发的人。

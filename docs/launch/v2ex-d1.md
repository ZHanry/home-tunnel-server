...
它不是公共代理，也不是给用户开放任意 FRP 配置。Agent 只接受控制中心签发、且通过本地白名单的 HTTP/HTTPS、已验证自定义域名和管理员授权 TCP 配置；UDP、访客模式、任意插件和通用 FRP 命令都会被拒绝。生产固定使用 FRP 0.62.1。0.70.1 已做隔离兼容验证，但在受保护的 Agent 重建、可信哈希更新和 Windows 10/11 VM 验证完成前不会切换。

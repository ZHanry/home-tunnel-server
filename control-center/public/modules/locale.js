import { state } from "./state.js?v=8.0.0-rc.1";
...
const zhToEn = {
  ...
  "无需家庭公网 IP 或路由器端口映射，受管发布 Web、通用 TCP 与固定端口 UDP 服务；RTSP-over-TCP 可直接使用通用 TCP 映射。":
    "Publish managed Web, general TCP, and fixed-port UDP services without a public IP at home or router port forwarding; RTSP-over-TCP uses a general TCP mapping.",
  ...
  "适合 NAS、家庭服务器和常开主机。图形界面请用同一包里的 home-tunnel-gui。":
    "For NAS devices, home servers, and always-on hosts. Use home-tunnel-gui from the same package for a graphical UI.",
  "同一套图形客户端 · Linux CLI/NAS 服务 · Android 远程管理":
    "One graphical client · Linux CLI/NAS service · Android remote management",
  隧道状态: "Tunnel status",
  ...
  "填写本地目标；端口隧道由管理员分配精确公网端口。":
    "Enter the local target; an administrator assigns the exact public port for raw-port tunnels.",
  "普通用户在网页控制台自助开通 HTTP；端口隧道由管理员分配精确公网端口。":
    "Standard users create HTTP tunnels in the web console; an administrator assigns the exact public port for raw-port tunnels.",
  云端管理: "Central management",
  按账号管理: "Per-account workspaces",
  "统一查看设备、状态和访问策略。": "Manage devices, status, and access policies in one place.",
  "每人只看见自己的设备与隧道。管理员管用户、配额和公网端口。":
    "Each person sees only their own devices and tunnels. Administrators manage users, quotas, and public ports.",
  ...
  "设备凭据写入 Windows Credential Manager":
    "Device credentials stored in Windows Credential Manager",
  ...
  新密码: "New password",
  "至少 12 个字符，且不能包含用户名。":
    "Use at least 12 characters and do not include the username.",
  ...
  "网关收到业务请求后会按 10 秒桶写入样本。":
    "The gateway writes samples in 10-second buckets after receiving traffic.",
  角色: "Role",
  状态: "Status",
  ...
  普通用户: "Standard user",
  还没有用户: "No users yet",
  "创建首个普通用户并将一次性临时密码安全交付给本人。":
    "Create the first standard user and securely deliver the one-time temporary password.",
  配置: "Configuration",
  最后在线: "Last seen",
  租约到期: "Lease expires",
  未知: "Unknown",
  ...
  "用户可通过 Windows 图形客户端或 Linux/macOS 无界面服务完成设备注册。":
    "Users can register devices with the Windows desktop client or Linux/macOS headless service.",
  归属: "Owner",
  访问控制: "Access control",
  本地目标: "Local target",
  ...
  "为已注册设备创建 HTTP/HTTPS 连接，或由管理员开启通用 TCP / UDP 端口隧道。":
    "Create an HTTP/HTTPS connection for a registered device, or let an administrator enable a general TCP / UDP port tunnel.",
  关键词: "Keywords",
  动作: "Action",
  目标类型: "Target type",
  每页: "Per page",
  ...
  "端口隧道仅管理员可分配，且不经过 HTTP 网关。":
    "Only administrators can assign raw-port tunnels; their traffic bypasses the HTTP gateway.",
  公网端口: "Public port",
  ...
  "先添加 TXT 所有权证明，再把域名 CNAME 到受管地址。验证成功后会自动申请证书并重配隧道。":
    "Add the TXT ownership proof, then CNAME the domain to the managed address. Successful verification triggers certificate issuance and tunnel reconfiguration.",
  ...
  "该用户全部活跃连接共享此上限；上传和下载共同消耗。":
    "All active connections for this user share the limit; upload and download consume the same pool.",
  ...
  "留空表示不限制来源。门禁在网关侧执行，保存后立即生效且不会重启隧道。":
    "Leave blank to allow all sources. Enforcement happens at the gateway and takes effect immediately without restarting the tunnel.",
  ...
  "全局关闭某种端口传输时，只允许停用既有连接或改回 HTTP。":
    "When a raw transport is globally disabled, an existing connection can only be disabled or changed back to HTTP.",
  ...
  "连接已创建；设备离线时保持 Pending":
    "Connection created; it remains pending while the device is offline",
  ...
  "HTTP 可自助开通 · TCP/UDP 由管理员分配端口":
    "Self-serve HTTP · TCP/UDP ports assigned by an administrator",
  累计流量: "Traffic so far",
  仅统计你名下的连接: "Only connections under your account",
  我的资源: "My resources",
  我的连接: "My connections",
  "从右上角创建 HTTP 隧道；设备需先在家里注册":
    "Create an HTTP tunnel from the top right; register a device at home first",
  还没有隧道: "No tunnels yet",
  "从右上角创建一条 HTTP 连接。设备需要先在家里注册。":
    "Create an HTTP connection from the top right after registering a device at home.",
  "在家里的机器上安装客户端并使用当前账号登录，设备会出现在这里。":
    "Install a client on a machine at home and sign in with this account. The device will appear here.",
  由客户端保活: "Kept alive by the client",
  请先在家里的机器上安装客户端并登录同一账号:
    "Install a client at home and sign in with the same account first",
  "创建 HTTP 隧道": "Create HTTP tunnel",
  公网子域: "Public subdomain",
  "普通用户只能自助创建 HTTP/HTTPS。TCP/UDP 由管理员分配精确公网端口。":
    "Standard users can only create HTTP/HTTPS tunnels. An administrator assigns exact TCP/UDP public ports.",
  "公网端口仍由管理员分配；你只能改自己的本地目标和访问控制。":
    "Public ports are still assigned by an administrator; you can only change your local target and access controls.",
  ...
  "临时密码 72 小时有效，首次登录后必须修改。关闭后无法再次查看。":
    "The temporary password is valid for 72 hours and must be changed on first sign-in. It cannot be viewed again after closing.",
  "关闭后无法再次查看；旧密码和全部旧会话已失效。":
    "It cannot be viewed again after closing. The old password and all previous sessions are invalid.",
  "已复制；请通过安全渠道交付": "Copied; deliver it through a secure channel",
  "浏览器未允许访问剪贴板，请手动选择并复制临时密码。":
    "Clipboard access was denied; select and copy the temporary password manually.",
  "请先让用户通过 Windows、Linux 或 macOS 客户端注册设备":
    "Ask the user to register a device with the Windows, Linux, or macOS client first",
  ...
  "一套部署只有一名管理员；此处只能创建普通用户。":
    "A deployment has a single administrator; only standard users can be created here.",
  子域命名策略: "Subdomain naming policy",
  "默认建议带用户名前缀，避免多人抢同一个短名。强制后新建 HTTP 子域必须以用户名开头。":
    "Names are suggested with a username prefix so people do not collide. Enforcing requires new HTTP subdomains to start with the username.",
  新建子域: "New subdomains",
  ...
  "忘记密码请联系创建该账号的管理员，控制台不提供网上自助重置。":
    "If you forgot the password, ask the administrator who created the account. This console does not offer self-service reset.",
  "使用本部署的管理员或普通用户账号继续。":
    "Continue with this deployment's administrator or a standard user account.",
  退出登录: "Sign out",
  "退出后需要重新输入账号密码。未保存的对话框内容会丢失。":
    "You will need to sign in again. Unsaved dialog content will be lost.",
  确认退出: "Sign out",
  ...
  我的账号: "My account",
  密码与使用额度: "Password and usage",
  我的账号与使用额度: "My account and usage",
  查找连接: "Find connections",
  所属用户: "Owner",
  全部用户: "All users",
  "输入名称、地址或用户": "Search name, address, or user",
  搜索: "Search",
  配置保存后自动同步到家庭设备: "Saved settings sync automatically to your home device",
  查看详情: "Details",
  暂停: "Pause",
  更多: "More",
  ...
  "本月 Web 流量": "Web traffic this month",
  月度配额: "Monthly quota",
  账号共享带宽: "Shared bandwidth",
  不限额: "Unlimited",
  "有新状态；完成编辑后点击刷新": "Updates available. Finish editing, then refresh.",
  "连接中断，显示上次同步的数据；点击刷新重试":
    "Connection interrupted. Showing previous data. Refresh to retry.",
  "实时连接已恢复，正在同步…": "Live connection restored. Syncing\u2026",
  "实时连接中断，显示上次同步的数据；正在重连…":
    "Live connection interrupted. Showing previous data. Reconnecting\u2026",
  访问保护与带宽设置: "Access protection and bandwidth",
  "这里的地址相对于所选家庭设备；127.0.0.1 指该设备本身。也可以粘贴完整 http(s) 地址自动填写协议与端口。":
    "This address is relative to the selected home device. 127.0.0.1 means that device. Paste an HTTP(S) URL to fill in the protocol and port.",
  "正在保存，请稍候。完成后会显示结果。": "Saving. Please wait for the result.",
  "已保留未提交的内容，重新打开可继续编辑": "Your draft is kept in this tab. Reopen to continue.",
  "已恢复上次未提交的内容，请核对后保存。": "Your draft was restored. Review it before saving.",
  读取最新版本并保留我的修改: "Load latest version and keep my edits",
  "已暂停。公网访问停止，启用后会重新应用配置。":
    "Paused. Public access is stopped. Enable to apply the configuration again.",
  "等待家庭设备上线。请检查电脑电源、客户端登录与网络。":
    "Waiting for your home device. Check power, client sign-in, and network.",
  "连接运行异常。请检查家庭设备上的目标服务和端口，再同步配置。":
    "Connection error. Check the service and port on your home device, then sync again.",
  "隧道已上线。请打开地址检查目标应用是否响应。":
    "Tunnel online. Open the address to check the application.",
  "配置已保存，等待家庭设备应用。若持续等待，请查看客户端状态。":
    "Saved. Waiting for your home device to apply the configuration. Check the client if this persists.",
  公网地址: "Public address",
  家庭设备: "Home device",
  本地服务: "Local service",
  ...
  知道了: "Done",
  "确认家庭设备开机，客户端已登录。": "Check that the home device is powered on and signed in.",
  "在家庭设备上访问本地服务地址，确认端口正确。":
    "Open the local service on the home device and check the port.",
  "确认配置已同步，再从外部网络检查公网地址。":
    "Confirm the configuration is synced, then check the public address from another network.",
  "检查应用自身的认证、加密和公网端口防火墙。":
    "Check application authentication, encryption, and the public-port firewall.",
  "如配置了 IP 白名单或访问口令，请确认访问条件。":
    "Check the allowed IP addresses and access password if configured.",
  安装客户端与快速开始: "Install a client and get started",
  创建第一条家庭连接: "Create your first home connection",
  "先在家庭电脑上安装客户端，用同一账号登录，然后把本地服务发布到公网。":
    "Install a client on your home computer and sign in with the same account, then publish a local service.",
  没有匹配的连接: "No matching connections",
  "请调整关键词或用户筛选后重试。": "Try another search or owner filter.",
  "这个用户还没有设备。请先安装客户端并用该账号登录。":
    "This user has no devices. Install a client and sign in with that account first.",
  确认删除这个域名: "Confirm domain removal",
  "删除后此域名将停止访问，原系统分配地址不受影响。再次点击确认删除。":
    "This domain will stop serving traffic. The system-assigned address remains available. Click again to confirm.",
  "密码修改后所有账号会话会退出，请使用新密码重新登录。":
    "Changing the password signs out all account sessions. Sign in again with the new password.",
};
...
export function translateTree(root = document, targetLocale = state.locale) {
  if (root.parentElement?.closest("[data-no-translate],[translate=no]")) return;
  ...
  for (const element of elements) {
    if (
      !element.matches("script,style,code") &&
      ...
      for (const attribute of ["aria-label", "title", "placeholder", "data-label"]) {
        if (element.hasAttribute(attribute)) {
          element.setAttribute(
            attribute,
            localizedText(element.getAttribute(attribute), targetLocale),
          );
        }
      }
    }
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      return node.parentElement?.closest("script,style,code,[data-no-translate],[translate=no]")

export function createConnectionsView({
  ...
  async function renderConnections(renderId = state.renderId) {
    ...
    <div class="actions"><button class="button button-secondary" data-action="batch-connections" data-enabled="false">暂停所选连接</button><button class="button button-secondary" data-action="batch-connections" data-enabled="true">恢复所选连接</button><span class="helper">每页独立选择，切页后清空</span></div>
  ...
  function renderConnectionCard(connection) {
    ...
    return `<article class="connection-card panel"><label><input type="checkbox" data-select-connection value="${connection.id}">选择此连接</label><div class="connection-heading"><div class="connection-identity"><span class="connection-symbol" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M10 13a5 5 0 0 0 7 .4l3-3a5 5 0 0 0-7-7l-2 2m3 6a5 5 0 0 0-7-.4l-3 3a5 5 0 0 0 7 7l2-2"/></svg></span><div><h3 data-no-translate>${escapeHtml(connection.name)}</h3><p data-no-translate>${escapeHtml(connection.device_name)}${isAdmin() ? ` · ${escapeHtml(connection.username)}` : ""}</p></div></div>${statusBadge(connection.enabled ? connection.state : "disabled")}</div>

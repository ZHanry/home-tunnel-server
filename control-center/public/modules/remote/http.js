...
export async function boundedResponse(response, maximum = 262144) {
  const reader = response.body?.getReader();
  if (!reader) throw new RemoteError("RD_PROTOCOL_MISMATCH");
  const chunks = []; let size = 0;
  try {
    while (true) {
      const { done, value } = await reader.read(); if (done) break;
      size += value.byteLength;
      if (size > maximum) throw new RemoteError("RD_MESSAGE_TOO_LARGE");
      chunks.push(value);
    }
    const data = new Uint8Array(size); let offset = 0;
    for (const chunk of chunks) { data.set(chunk, offset); offset += chunk.byteLength; }
    return strictJson(new TextDecoder("utf-8", { fatal: true }).decode(data), maximum);
...
export class RemoteApi {
  ...
  async initialize() {
    ...
    if (!this.identity.endpointId) {
      const challenge = await this.account("/api/v1/rd/enrollment-challenges", { method: "POST", body: JSON.stringify({ endpoint_kind: "browser", role: "controller", public_jwk: this.identity.jwk }) });
      ...
      const result = await this.account("/api/v1/rd/endpoints", { method: "POST", body: JSON.stringify({ challenge_id: challenge.challenge_id, signed_proof: await signJws(this.identity, expected), name: "Web browser", platform: "browser" }) });
  ...
  async refresh() {
    if (!this.refreshing) {
      this.refreshing = (async () => {
        const challenge = await this.account("/api/v1/rd/token-challenges", { method: "POST", body: JSON.stringify({ endpoint_id: this.identity.endpointId, purpose: "controller_refresh" }) });
        ...
        const result = await this.account("/api/v1/rd/tokens", { method: "POST", body: JSON.stringify({ challenge_id: challenge.challenge_id, endpoint_id: this.identity.endpointId, proof: await signJws(this.identity, challenge.proof_payload) }) });
        this.acceptToken(result);
  ...
  async request(path, { method = "GET", body, idempotencyKey } = {}) {
    ...
    const response = await fetch(path, { method, credentials: "same-origin", redirect: "error", headers, body: body === undefined ? undefined : JSON.stringify(body), signal: AbortSignal.any([this.abort.signal, AbortSignal.timeout(20000)]) });

...
test("public landing page stays available while Windows release metadata is absent", async () => {
  ...
  try {
    ...
    assert.match(landing.body.toString("utf8"), /app\.js\?v=6\.0\.1/);
    assert.match(landing.body.toString("utf8"), /type="module"/);
    assert.match(landing.body.toString("utf8"), /console\.css\?v=6\.0\.1/);
    assert.match(landing.body.toString("utf8"), /theme\.js\?v=6\.0\.1/);
    ...
    const stylesheet = await request(origin + "/console.css?v=6.0.1");
    assert.equal(stylesheet.status, 200);
    assert.equal(stylesheet.headers["cache-control"], "public, max-age=31536000, immutable");

    const themeScript = await request(origin + "/theme.js?v=6.0.1");
    assert.equal(themeScript.status, 200);
    assert.match(themeScript.body.toString("utf8"), /ht_locale/);

    const applicationScript = await request(origin + "/app.js?v=6.0.1");
    assert.equal(applicationScript.status, 200);
    assert.match(applicationScript.body.toString("utf8"), /\.\/modules\/api\.js\?v=6\.0\.1/);
    assert.match(applicationScript.body.toString("utf8"), /\.\/modules\/locale\.js\?v=6\.0\.1/);
    assert.match(applicationScript.body.toString("utf8"), /\.\/modules\/realtime\.js\?v=6\.0\.1/);
    ...
    const localeModule = await request(origin + "/modules/locale.js?v=6.0.1");
    ...
    const realtimeModule = await request(origin + "/modules/realtime.js?v=6.0.1");

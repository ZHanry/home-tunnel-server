...
op('get','/client/remote-devices',page(ref('RemoteDevice')),params=pagination,description="Read-only same-account device directory for the desktop remote launcher. Device sessions cannot query another owner or mutate peer devices; the existing /client/devices endpoint remains device-scoped.")
...
document={"openapi":"3.1.0","info":{"title":"Home Tunnel API","version":"1.3.0","description":"Home Tunnel 9.0.0 API contract. Adds three remote-access authorization modes; direct UDP media is never relayed here. Existing tunnel WebSocket envelopes remain in home-tunnel.v1.json; RD wire registry is remote-desktop.v1.json. See docs/API.md for authentication, compatibility and replay semantics.","license":{"name":"Apache-2.0"}},"servers":[{"url":"https://console.example.com"}],"security":[{"bearerAuth":[]},{"sessionCookie":[]}],"paths":paths,"components":{"securitySchemes":{"bearerAuth":{"type":"http","scheme":"bearer"},"sessionCookie":{"type":"apiKey","in":"cookie","name":"ht_access"},"internalKey":{"type":"apiKey","in":"header","name":"x-home-tunnel-key"},"dpopAuth":{"type":"http","scheme":"DPoP","description":"Short-lived endpoint token; requires a matching DPoP proof."},"dpopProof":{"type":"apiKey","in":"header","name":"DPoP","description":"ES256 proof binds token hash, nonce, HTTP method, canonical URL, timestamp and unique jti."}},"schemas":schemas},"x-contract-ref":"api-v1.3.0"}
encoded=json.dumps(document,ensure_ascii=False,indent=2)+'\n'
json_schema={"$schema":"https://json-schema.org/draft/2020-12/schema","$id":"https://zhanry.github.io/home-tunnel/schemas/api-v1.3.0.json","$defs":schemas}

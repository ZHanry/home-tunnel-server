...
def install(context):
    ...
    define("RdAssistInvite", {"id": ID, "device_id": device_code, "temporary_password": {**string(12, 12), "readOnly": True}, "expires_at": DATE}, ["id", "device_id", "temporary_password", "expires_at"])
    define("RdAssistTarget", {"invite_id": ID, "host_endpoint_id": ID, "host_owner_user_id": ID, "host_name": S, "host_jkt": nonce, "host_public_jwk": ref("RdPublicKey"), "capabilities": capability, "expires_at": DATE}, ["invite_id", "host_endpoint_id", "host_owner_user_id", "host_name", "host_jkt", "host_public_jwk", "capabilities", "expires_at"])
    define("RdAssistInviteSummary", {"id": ID, "device_id": device_code, "state": enum("active", "redeemed"), "expires_at": DATE, "created_at": DATE, "redeemed_at": nullable(DATE)}, ["id", "device_id", "state", "expires_at", "created_at", "redeemed_at"])
    define("RdAccessProfile", {"device_id": nullable(device_code), "fixed_password_enabled": B, "revision": I}, ["device_id", "fixed_password_enabled", "revision"])
    define("RdAccessRequest", {"id": ID, "state": enum("pending", "preparing", "approved", "rejected", "expired"), "expires_at": DATE, "target": ref("RdAssistTarget")}, ["id", "state"])
    define("RdAccessDecision", {"id": ID, "state": enum("preparing", "approved", "rejected"), "invite_id": nullable(ID), "expires_at": nullable(DATE)}, ["id", "state"])
    define("RdAccessRequestSummary", {"id": ID, "controller_endpoint_id": ID, "expires_at": DATE}, ["id", "controller_endpoint_id", "expires_at"])
    define("RdAccessInviteAuthorization", {"invite_id": ID, "access_kind": {"const": "fixed_password"}, "profile_revision": V, "expires_at": DATE}, ["invite_id", "access_kind", "profile_revision", "expires_at"])
    ...
    route("post", "/rd/assist-invites", ref("RdAssistInvite"), status=201, description="The local host creates a 12-character one-time password and nine-digit device code. Password is returned once, expires after five minutes, and is never stored in plaintext.")
    route("get", "/rd/assist-invites", obj({"items": array(ref("RdAssistInviteSummary"), 30)}, ["items"]), description="The originating host lists active and redeemed invitations without retrieving the one-time password.")
    route("post", "/rd/assist-invites/redeem", ref("RdAssistTarget"), strict({"device_id": device_code, "temporary_password": string(128, 1)}, ["device_id", "temporary_password"]), description="A controller on the same server may redeem once. Five wrong attempts revoke the invitation. Cross-account pairing must present this invitation ID and is limited to one signed session.")
    route("delete", "/rd/assist-invites/{id}", status=204, description="The originating host revokes an active or redeemed invitation immediately.")
    route("get", "/rd/access-profile", ref("RdAccessProfile"), description="Host-only stable nine-digit cross-account device ID and fixed-password status; the password is never returned.")
    route("post", "/rd/access-profile", ref("RdAccessProfile"), status=201, description="Host-only idempotent creation of a stable cross-account device ID.")
    route("put", "/rd/access-profile/password", ref("RdAccessProfile"), strict({"password": string(128, 12)}, ["password"]), description="Host-only Argon2id fixed password setup or rotation. Old fixed-password invitations and their sessions are revoked.")
    route("delete", "/rd/access-profile/password", ref("RdAccessProfile"), description="Host-only fixed password disable, immediately revoking fixed-password invitations and sessions.")
    route("post", "/rd/access/fixed/redeem", ref("RdAssistTarget"), strict({"device_id": device_code, "password": string(128, 1)}, ["device_id", "password"]), description="Any authenticated controller on this server may use the fixed password, without prebinding to the host account. Five failures lock the device for five minutes; successful redemption creates one five-minute, single-pairing signed invitation.")
    route("post", "/rd/access/requests", ref("RdAccessRequest"), strict({"device_id": device_code}, ["device_id"]), 201, description="An authenticated cross-account controller requests one connection; only an explicit local host decision can approve it. Request expires after two minutes.")
    route("get", "/rd/access/requests", obj({"items": array(ref("RdAccessRequestSummary"), 5)}, ["items"]), description="Host-only pending request list.")
    route("get", "/rd/access/requests/{id}", ref("RdAccessRequest"), description="Controller-only request status. The target is revealed only after host local trust persistence and activation.")
    route("post", "/rd/access/requests/{id}/decision", ref("RdAccessDecision"), strict({"decision": enum("approve", "reject")}, ["decision"]), description="Host-only local decision. Approval prepares a single-pairing invitation but does not reveal it to the controller until activation.")
    route("post", "/rd/access/requests/{id}/activate", ref("RdAccessDecision"), description="Host activates an approved request after persisting local authorization.")
    route("get", "/rd/access/invites/{id}/authorization", ref("RdAccessInviteAuthorization"), description="Host-only fixed-password redemption provenance, bound to the current password revision and invitation expiry.")
    route("post", "/rd/pairings", ref("RdPairing"), strict({"host_endpoint_id": ID, "session_request_id": ID, "permissions": scope, "mode": enum("one_session", "persistent"), "nonce_controller": nonce, "assist_invite_id": ID}, ["host_endpoint_id", "session_request_id", "permissions", "nonce_controller"]), 201)
    ...
    define("RdReconnectInput", {"expected_epoch": {"type": "integer", "minimum": 1, "maximum": 4294967295}, "reason": enum("network_changed", "ice_failed", "display_changed", "media_failed"), "display_id": string(128, 1)}, ["expected_epoch", "reason"])
    schemas["RdReconnectInput"]["additionalProperties"] = False
    schemas["RdReconnectInput"]["oneOf"] = [{"properties": {"reason": {"const": "display_changed"}}, "required": ["display_id"]}, {"properties": {"reason": enum("network_changed", "ice_failed", "media_failed")}, "not": {"required": ["display_id"]}}]
    route("post", "/rd/sessions/{id}/reconnect", ref("RdSession"), ref("RdReconnectInput"), 202, idempotent=True, description="Original controller requests a new PeerConnection and connection epoch using expected_epoch and Idempotency-Key. display_changed requires display_id selecting a different advertised, ready host display; other reasons omit display_id and retain the current display, which must still exist. The display and epoch change atomically. Host approval requires stopping the old capture/input epoch before accepting the new ticket. The original lease and deadlines are not extended. Network/media recovery is limited to three rebuilds per session independently of display switching. Epochs cannot exceed uint32; normal request rate limits apply. Same key/body replays the persisted request without another epoch change; a changed body conflicts.")
